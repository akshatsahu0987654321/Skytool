...
(function () {
  ...
  // ── State ──────────────────────────────────────────────────────────────
  var state = {
    rows: [],          // [{id, value}] — proxy list currently being edited
    mode: "round_robin",
  ...
  function renderRows() {
    if (state.rows.length === 0) {
      dom.rowsHost.innerHTML =
        '<div class="proxy-pool-empty muted">No proxies yet. Click "Add proxy" or "Bulk paste".</div>';
      updateSummary();
      return;
    }
    var html = state.rows
      .map(function (row, idx) {
        ...
        return (
          '<div class="proxy-pool-row" data-row-id="' + row.id + '">' +
            ...
            '<span class="' + dotCls + '" title="' + escHtml(statusTxt || "not tested") + '"></span>' +
            ...
            '<button class="icon-btn icon-danger proxy-pool-remove" data-row-id="' + row.id +
              '" type="button" title="Delete" aria-label="Delete proxy">' +
  ...
  function load() {
    ...
    return api("/api/proxy/pool")
      .then(function (data) {
        ...
        var rt = data.runtime || {};
        if (rt.total) {
          setStatus("Saved " + rt.total + " proxy · " + (rt.live || 0) + " live.", null);
        }
      })
      .catch(function (err) {
  ...
  function save() {
    ...
    api("/api/proxy/pool", {
      ...
      .then(function (data) {
        ...
        renderRows();
        var extra = data.settings_persist_error ? " (warning: " + data.settings_persist_error + ")" : "";
        setStatus("Saved " + proxies.length + " proxy." + extra, data.settings_persist_error ? "fail" : "ok");
      })
      .catch(function (err) {
        setStatus("Save failed: " + err.message, "fail");
      })
      .finally(function () {
        state.busy = false;
  ...
  function testAll() {
    ...
    var proxies = collectProxies();
    if (proxies.length === 0) {
      setStatus("No proxies to test.", "fail");
      return;
    }
    state.busy = true;
    dom.btnTestAll.disabled = true;
    setStatus("Testing " + proxies.length + " proxy...", null);
    api("/api/proxy/test-all", {
      ...
      .then(function (data) {
        ...
        renderRows();
        setStatus(
          "Test complete: " + (data.live || 0) + " live / " + (data.dead || 0) + " dead / " + (data.total || 0) + " total.",
          (data.dead || 0) > 0 ? "fail" : "ok"
        );
      })
      .catch(function (err) {
        setStatus("Test failed: " + err.message, "fail");
      })
      .finally(function () {
        state.busy = false;
  ...
  function setTgBadge(configured) {
    ...
    dom.tgBadge.textContent = configured ? "configured" : "not configured";
  ...
  function loadLoginFlow() {
    ...
    return api("/api/settings/session.login_flow")
      ...
      .catch(function () {
        dom.loginFlow.value = "anti409";  // 404/not set → default
      });
  }
  ...
  function saveLoginFlow() {
    ...
    api("/api/settings/session.login_flow", {
      ...
      .catch(function (err) {
        setStatus("Failed to save login flow: " + err.message, "fail");
      });
  }
  ...
  function loadTelegram() {
    ...
    return api("/api/telegram/config")
      ...
      .catch(function (err) {
        setTgStatus("Load failed: " + err.message, "fail");
      });
  }
  ...
  function renderTunnelSnapshot(snap) {
    ...
    dom.tunToggleLabel.textContent = enabled ? "On" : "Off";

    dom.tunUrlInput.value = url;
    dom.tunUrlInput.placeholder = enabled
      ? (status === "starting" ? "Requesting URL from Cloudflare..." : "(no URL yet)")
      : "(tunnel is off)";
    ...
    if (snap.error) {
      setTunStatusLine("Error: " + snap.error, "fail");
    } else if (status === "running" && snap.uptime_sec != null) {
      setTunStatusLine("Running · uptime " + snap.uptime_sec + "s · points to " +
        (snap.local_host + ":" + snap.local_port), "ok");
    } else if (status === "starting") {
      setTunStatusLine("Starting cloudflared...", null);
    } else if (status === "stopped") {
      setTunStatusLine("Tunnel is off.", null);
    }

    if (dom.tunLogTail) {
      var lines = (snap.log_tail || []).join("\n");
      dom.tunLogTail.textContent = lines || "(no log yet)";
    }
  }
  ...
  function refreshTunnelStatus() {
    return api("/api/tunnel/status")
      ...
      .catch(function (err) {
        setTunStatusLine("Failed to load tunnel status: " + err.message, "fail");
      });
  }
  ...
  function toggleTunnel() {
    ...
    setTunStatusLine(want ? "Turning tunnel on (first run downloads cloudflared)..." : "Turning tunnel off...", null);
    api("/api/tunnel/config", {
      ...
      .then(function (snap) {
        ...
        if (snap.settings_persist_error) {
          setTunStatusLine("Applied, but DB save failed: " + snap.settings_persist_error, "fail");
        }
      })
      .catch(function (err) {
        setTunStatusLine("Toggle failed: " + err.message, "fail");
  ...
  function restartTunnel() {
    ...
    setTunStatusLine("Requesting a new URL...", null);
    api("/api/tunnel/restart", { method: "POST" })
      .then(function (snap) {
        renderTunnelSnapshot(snap);
        if (snap.status === "running") {
          setTunStatusLine("New URL issued.", "ok");
        }
      })
      .catch(function (err) {
        setTunStatusLine("Restart failed: " + err.message, "fail");
      })
      .finally(function () {
        tunState.busy = false;
  ...
  function copyTunnelUrl() {
    ...
    if (!url) return;
    var done = function () { setTunStatusLine("Copied URL.", "ok"); };
  ...
  function saveTelegram() {
    ...
    api("/api/telegram/config", {
      ...
      .then(function (data) {
        setTgBadge(!!data.configured);
        var extra = data.persist_error ? " (warning: " + data.persist_error + ")" : "";
        setTgStatus("Saved." + extra, data.persist_error ? "fail" : "ok");
      })
      .catch(function (err) {
        setTgStatus("Save failed: " + err.message, "fail");
      })
      .finally(function () {
        tgState.busy = false;
  ...
  function testTelegram() {
    ...
    api("/api/telegram/config", {
      ...
      .then(function () { setTgStatus("Test message sent — check Telegram.", "ok"); })
      .catch(function (err) { setTgStatus("Test failed: " + err.message, "fail"); })
      .finally(function () {
        tgState.busy = false;
  ...
  function applyPaste() {
    ...
    renderRows();
    closePaste();
    setStatus("Added " + added + " proxy. Remember to click Save.", null);
  }
  ...
  function init() {
    cacheDom();
    if (!dom.section) return; // tab does not exist
    bindEvents();
