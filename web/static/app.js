...
(() => {
  ...
  function persistTextarea(key, value) {
    try { localStorage.setItem(key, value); } catch (e) { /* quota ignored */ }
  }
  ...
  const state = {
    ...
    order: [],                // job id order
    activeJobId: null,        // job currently selected for log view
    maxConcurrent: 3,
    mode: 'multi',
    headless: true,
    debug: false,
  ...
  const mailModeUiCopy = Object.freeze({
    icloud_v3: {
      input_help: 'One iCloud v3 entry per line: email|api_url (Worker v2 readmail URL). Need iCloud mail? Contact @skywalker1_1 on Telegram (https://t.me/skywalker1_1) to buy.',
      input_placeholder: '⚡ Contact @skywalker1_1 on Telegram (https://t.me/skywalker1_1) to buy iCloud mail.\n\nFormat — one entry per line: email|api_url\nExample:\npetunia-boar-3d+hblx3n@icloud.com|https://icloud-cf-mail-v2.n5pskgzs9g.workers.dev/readmail/<token>/data',
    },
    outlook: {
    ...
    worker: {
      input_help: 'One iCloud email per line via Worker OTP. Need iCloud mail? Contact @skywalker1_1 on Telegram (https://t.me/skywalker1_1) to buy.',
      input_placeholder: '⚡ Contact @skywalker1_1 on Telegram (https://t.me/skywalker1_1) to buy iCloud mail.\n\nFormat — one iCloud email per line:\nuser@icloud.com',
    },
    gmail_advanced: {
      input_help: 'One line: api_url or email|api_url. Pre-check mail_status=live.',
  ...
  function copyWithToast(text, message, opts) {
    return copyText(text).then(() => {
      toast(message || 'Copied', Object.assign({ type: 'success' }, opts || {}));
    }).catch((err) => {
  ...
  document.getElementById('btn-clear-all').addEventListener('click', async () => {
    if (!(await Dialog.confirm({ message: 'Delete ALL jobs in every status? This cannot be undone.', danger: true, confirmLabel: 'Delete' }))) return;
    try {
  ...
  dom.modeSelect.addEventListener('change', async () => {
    ...
    const cfg = MODE_TAB_CONFIG[tabId];
    if (!cfg) return; // tab has no mode (settings) — ignore
  ...
  dom.headlessToggle.addEventListener('change', async () => {
    ...
    if (runningCount > 0) {
      const ok = await Dialog.confirm({ message:
        `There are ${runningCount} RUNNING job(s). Changing Headless will not ` +
        `apply to those jobs because their browsers already launched. It only affects new jobs.\n\n` +
        `Continue switching to ${headless ? 'Headless' : 'Headed'}?`
      });
      if (!ok) {
  ...
  (async () => {
    ...
    try {
      ...
      console.error('GET /api/config failed, using Settings DB fallback:', err);
    }
