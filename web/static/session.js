...
(() => {
  ...
  function renderJobs() {
    ...
    const html = state.order.map((id, idx) => {
      ...
      if (j.status === 'running') {
        ...
        actionBtns = `
          <button class="icon-btn" data-action="reload" data-id="${escHtml(id)}" title="Reload session (reload to see the latest account type)">${window.GptUi.icon('retry')}</button>
  ...
  function _formatAccountLine(j) {
    ...
    if (!password) {
      return `${j.email}  (loading secrets...)`;
    }
    return secret
